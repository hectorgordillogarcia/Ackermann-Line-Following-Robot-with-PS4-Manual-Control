...
class F1Node(Node):
    ...
    def __init__(self, max_speed=3.0) -> None:
        super().__init__('f1_node')
        self.max_speed = max_speed

        self.linear_controller = PID(k_p=1.0, k_d=0.0)
        # self.linear_controller.setPoint(max_speed)
        self.linear_controller.setpoint = 0.0
        self.angular_controller = PID(k_p=0.25, k_d=0.0)
    ...
    def img_callback(self, msg: Image) -> None:
        ...
        if centroid is None:
            self.get_logger().warn("No red line detected!")
            return
    ...
    def send_cmd_vel(self, error):
        ...
        print(f"{error=}")
        print(f"{self.max_speed - abs(linear_vel)}", f"{angular_vel=}")
